...
/*   Updated: 2025/03/03 16:10:16 by dbaladro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void readButton(void) {
	...
	uint8_t status = PIND & (1 << PD2); /* When the button is pressed it connects
										*  to the ground  setting it to 0
										*  We then isolate the necessary bit with a mask
										*/
	if (status == 0) /// Depending on the pin status we turn on or off the led
		PORTB |= (1 << PB0);
	else
		PORTB &= ~(1 << PB0);
}
